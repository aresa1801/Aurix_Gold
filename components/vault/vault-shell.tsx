...
import { useVaultData } from "@/components/vault/use-vault-data"
...
export function VaultShell({ children, title, eyebrow }: { children: React.ReactNode; title: string; eyebrow?: string }) {
  ...
  const { vault, loading } = useVaultData()
...
export function QuickActions() { const { requestAction } = useVaultData(); const actions = [{ label: "Add Reserve", action: "add_reserve", icon: Plus }, { label: "Withdraw Revenue", action: "withdraw_revenue", icon: ArrowDownToLine }, { label: "Update PoR Feed", action: "update_por", icon: Activity }, { label: "Generate Report", action: "generate_report", icon: FileText }, { label: "Contact Support", action: null, icon: CircleHelp }]; const submit = async (label: string, action: string | null) => { if (!action) { window.alert("Support request flow opened"); return } try { await requestAction(action); window.alert(`${label} request submitted`) } catch { window.alert("Unable to submit request. Check vault access.") } }; return <div className="flex flex-wrap gap-2 rounded-xl border border-white/[0.08] bg-white/[0.025] p-3">{actions.map(({ label, action, icon: Icon }) => <Button key={label} variant="ghost" className="text-xs text-white/60 hover:bg-[#d6b46a]/10 hover:text-[#e3c57d]" onClick={() => void submit(label, action)}><Icon data-icon="inline-start" />{label}</Button>)}</div> }
