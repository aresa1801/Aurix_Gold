...
export default function SwapPage() {
  ...
  const [transactionSteps, setTransactionSteps] = useState([
    {
      ...
      name: "Token Approval",
      status: "pending",
      description: "Approve IDRT spending",
    },
    {
      id: 2,
      name: "Execute Swap",
      status: "pending",
      description: "Convert tokens using smart contract",
    },
    {
      id: 3,
      name: "Update Balances",
      status: "pending",
      description: "Refresh wallet balances",
    },
  ]);
  ...
  console.log(
    "💰 Swap page using gold price:",
    goldPriceIDR,
    isUsingFallback ? "(FALLBACK)" : "(LIVE)",
  );
  ...
  const loadContractData = async () => {
    ...
    try {
      ...
      const [feePercent, idrtBalance, gTokenBalance] = await Promise.all([
        ...
        userAddress
          ? contractService.getIDRTBalance(userAddress).catch(() => "0")
          : Promise.resolve("0"),
        userAddress
          ? contractService.getGoldTokenBalance(userAddress).catch(() => "0")
          : Promise.resolve("0"),
      ]);
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      if (fromToken === "IDRT" && toToken === "G-TOKEN") {
        ...
        const newIdrtBalance = Math.max(
          0,
          Number.parseFloat(userBalances.idrt) - Number.parseFloat(amount),
        );
        const newGTokenBalance =
          Number.parseFloat(userBalances.gToken) +
          Number.parseFloat(calculatedToAmount);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen px-4 py-8">
        <div className="container mx-auto flex max-w-lg items-center justify-center">
          <div className="w-full rounded-2xl border border-soft-white/5 bg-navy-800/30 p-8 text-center backdrop-blur-md">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-2 border-soft-white/20 border-t-gold"></div>
            <p className="text-sm text-soft-white/70">Loading swap data...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (!isLoading) {
    ...
    console.log(
      `   Gold Price: ${goldPriceIDR} IDR/gram ${isUsingFallback ? "(FALLBACK)" : "(LIVE)"}`,
    );
    console.log(
      `   Platform Fee (${swapFeePercent}%): ${testFee.toLocaleString()} IDRT`,
    );
      ...
      <div className="min-h-screen px-4 py-8">
        <div className="container mx-auto max-w-6xl space-y-8">
          <div className="space-y-3 text-center">
            <h1 className="text-4xl font-bold text-soft-white">
              {t("swap.title")}
            </h1>
            <p className="text-soft-white/50">{t("swap.subtitle")}</p>
          </div>
          ...
          <Card className="mx-auto max-w-4xl rounded-2xl border border-soft-white/5 bg-navy-800/30 backdrop-blur-md shadow-xl shadow-black/10">
            <CardHeader>
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-soft-white/50">Input Amount:</span>
                    <span className="text-gold font-semibold">
                      1,000,000 IDRT
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-soft-white/50">Gold Price:</span>
                    <span
                      className={`font-semibold ${isUsingFallback ? "text-orange-400" : "text-prosperity"}`}
                    >
                      {formatCurrency(goldPriceIDR)} IDR/gram
                      {isUsingFallback && (
                        <span className="text-xs ml-1">(FALLBACK)</span>
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-soft-white/50">
                      Platform Fee ({swapFeePercent}%):
                    </span>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-soft-white/50">After Fee:</span>
                    <span className="text-soft-white font-semibold">
                      {formatCurrency(1000000 * (1 - swapFeePercent / 100))}{" "}
                      IDRT
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-soft-white/50">Gold Equivalent:</span>
                    <span className="text-gold font-semibold">
                      {(
                        (1000000 * (1 - swapFeePercent / 100)) /
                        goldPriceIDR
              ...
              <div className="rounded-2xl border border-soft-white/5 bg-navy-900/40 p-4">
                <div className="mb-2 text-sm text-soft-white/50">
                  Calculation Formula:
                </div>
              ...
              {isUsingFallback && (
                <div className="rounded-2xl border border-orange-500/20 bg-orange-500/10 p-4">
                  ...
                  <div className="text-orange-300 text-xs mt-1">
                    Using reliable fallback price of 1,085,000 IDR per gram. All
          ...
          <div className="mx-auto max-w-lg space-y-6">
            <Card className="rounded-2xl border border-soft-white/5 bg-gradient-to-br from-navy-800/55 via-navy-800/35 to-gold/5 backdrop-blur-md shadow-2xl shadow-black/20">
              <CardHeader className="space-y-2">
                <CardTitle className="text-gold">
                  {t("swap.tokenSwap")}
                </CardTitle>
                <CardDescription className="text-soft-white/50">
                  {t("swap.tokenSwapDesc")}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* From Token */}
                <div className="space-y-2">
                  <Label className="text-soft-white/50">{t("swap.from")}</Label>
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <Input
                        ...
                        className="h-14 rounded-xl border-soft-white/5 bg-navy-900/40 text-lg text-soft-white placeholder:text-soft-white/30"
                      />
                    </div>
                    <Select value={fromToken} onValueChange={setFromToken}>
                      <SelectTrigger className="h-14 w-32 rounded-xl border-soft-white/5 bg-navy-900/40 text-soft-white">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="border-soft-white/5 bg-navy-800/95 backdrop-blur-md">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-soft-white/50">
                      {t("swap.balance")}: {getTokenBalance(fromToken)}{" "}
                      {fromToken}
                    </div>
                    <Button
                      variant="ghost"
                      ...
                      className="h-auto px-2 text-xs text-prosperity hover:text-prosperity/80"
                    >
                      Demo: 1M IDRT
                    </Button>
                  </div>
                ...
                <div className="flex justify-center">
                  <Button
                    ...
                    size="icon"
                    onClick={swapTokens}
                    className="h-11 w-11 rounded-full border border-soft-white/5 bg-soft-white/5 text-gold hover:bg-soft-white/10"
                  >
                ...
                {/* To Token */}
                <div className="space-y-2">
                  <Label className="text-soft-white/50">{t("swap.to")}</Label>
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <Input
                        type="text"
                        ...
                        readOnly
                        className="h-14 rounded-xl border-soft-white/5 bg-navy-900/40 text-lg text-soft-white"
                      />
                    </div>
                ...
                {/* Exchange Rate */}
                <div className="space-y-4 rounded-2xl border border-soft-white/5 bg-navy-900/40 p-4">
                  <div className="flex items-center justify-between">
                    <span className="text-soft-white/50">
                      {t("swap.exchangeRate")}
                    </span>
                    <span className="text-gold font-mono">
                      1 G-TOKEN = Rp {formatCurrency(goldPriceIDR)}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-1">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="border-soft-white/5 bg-navy-800/95 backdrop-blur-md">
                          <p>
                            {swapFeePercent}% fee supports platform operations
                            and staking rewards
                          </p>
                        </TooltipContent>
                      </Tooltip>
                  ...
                  <Separator className="bg-soft-white/5" />
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-soft-white">
                      {fromToken === "IDRT" ? "Rp " : ""}
                      {formatCurrency(Number(calculateFee()))}{" "}
            ...
            {/* Transaction Preview */}
            {showPreview && (
              <Card className="rounded-2xl border border-soft-white/5 bg-navy-800/30 backdrop-blur-md shadow-xl shadow-black/10">
                <CardHeader className="space-y-2">
                  <CardTitle className="text-gold">
                    {t("swap.transactionPreview")}
                  </CardTitle>
                  <CardDescription className="text-soft-white/50">
                    {t("swap.transactionPreviewDesc")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="space-y-4 rounded-2xl border border-soft-white/5 bg-navy-900/40 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-soft-white font-semibold">
                          {fromToken === "IDRT" ? "Rp " : ""}
                          {formatCurrency(Number(fromAmount))}{" "}
                          {getTokenSymbol(fromToken)}
                        </div>
                        {fromToken === "G-TOKEN" && (
                          <div className="text-sm text-soft-white/50">
                            ≈ {Number(fromAmount).toFixed(3)}g Gold
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-gold font-semibold">
                          {toToken === "IDRT" ? "Rp " : ""}
                          {formatCurrency(Number(calculateToAmount()))}{" "}
                    ...
                    <div className="rounded-2xl border border-soft-white/5 bg-navy-900/40 p-4">
                      <div className="mb-4 flex items-center justify-between">
                        ...
                        <div className="text-xs text-soft-white/50">
                          {
                            transactionSteps.filter(
                              (step) => step.status === "completed",
                            ).length
                          }
                          /{transactionSteps.length} complete
                        </div>
                      </div>
                      <div className="space-y-4">
                        {transactionSteps.map((step, index) => (
                          <div key={step.id} className="flex gap-3">
                                  ...
                                  step.status === "completed"
                                    ...
                                    : step.status === "processing"
                                      ...
                                      : step.status === "failed"
                                        ...
                                        : "border border-soft-white/10 bg-soft-white/5 text-soft-white/50"
                                }`}
                              >
                                {step.status === "completed" ? (
                                  "✓"
                            ...
                            <div className="flex-1 rounded-xl border border-soft-white/5 bg-navy-800/30 px-4 py-3">
                              <div
                  ...
                  <div className="flex gap-3">
                    <Button
                      variant="outline"
                      className="flex-1 rounded-xl border-soft-white/5 bg-transparent text-soft-white hover:bg-soft-white/5"
                    ...
                    <Button
                      className="flex-1 rounded-xl bg-gold font-semibold text-navy-900 hover:bg-gold-600"
                      onClick={executeSwap}
                      disabled={isSwapping}
                    >
                      {isSwapping ? (
                        <span className="flex items-center justify-center gap-2">
                          <span className="h-3 w-3 animate-spin rounded-full border-2 border-navy-900/40 border-t-navy-900" />
                          Swapping...
                        </span>
                      ) : (
                        t("swap.confirmSwap")
